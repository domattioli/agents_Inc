"""T004: tests for agents_inc/config_schema.py -- stdlib dataclass validation."""
...
from agents_inc import config_schema as cs  # noqa: E402
...
class TestConfigSchema(unittest.TestCase):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            for name in ("governance", "models", "protocols", "routing"):
                (tdp / f"{name}.json").write_text(
                    (ROOT / "agents_inc" / f"{name}.json").read_text()
                )
    ...
    def test_no_third_party_dependency_added(self):
        result = subprocess.run(
            [sys.executable, "-c", "import agents_inc.config_schema"],
            cwd=str(ROOT),
            capture_output=True,
            text=True,
        )
        ...
        # Parse config_schema.py and check all imports are stdlib or agents_inc
        config_schema_path = ROOT / "agents_inc" / "config_schema.py"
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Import):
                for alias in node.names:
                    ...
                    if module_name not in sys.stdlib_module_names and module_name != "agents_inc":
                        offenders.append(module_name)
            elif isinstance(node, ast.ImportFrom):
                if node.module:
                    module_name = node.module.split('.')[0]
                    if module_name != "__future__" and module_name not in sys.stdlib_module_names and module_name != "agents_inc":
                        offenders.append(module_name)
