...
class DoctorReport:
    ...
    codes: tuple[str, ...]
    live: dict | None = None
    def as_dict(self): return {"ready": self.ready, "codes": list(self.codes), "live": self.live}
...
def check_install(paths: InstallPaths, live_model: str | None = None, runner=subprocess.run) -> DoctorReport:
    ...
    if not receipt.codex_path.is_file(): codes.append("WB_CLI_NOT_FOUND")
    ...
    return DoctorReport(not codes, tuple(sorted(set(codes))), live)
